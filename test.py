import requests
from requests.auth import HTTPBasicAuth
import json


# url_post = "http://10.10.10.138:1337/api/approved-packages"
# token = "bearer "

# new_package = {
#     "name": "testname",
#     "identifier": "testid",
#     "description": "testdesc",
#     "versions": [16.4,20.2],
#     "path": "/b/joe"
# }

# payload = {
#     "data": new_package
# }

# post_response = (requests.post(url=url_post, json=payload, headers={"Authorization": token, "Content-Type": "application/json"}))
# post_response_json = post_response.json()
# print(post_response_json)

url_post = "http://101.188.67.134:1337/api/approved-packages"

token = "bearer "

updated_package = {
    "name": "name",
    "identifier": "pid",
    "description": "descr",
    "versions": [1.3, 1.4],
    "path": "https://"
}
payload = {
    "data": updated_package
}
post_response = requests.get(url=url_post, json=payload, headers={"Authorization": token, "Content-Type": "application/json"})
# Print the response
post_response_json = post_response.json()
print(post_response_json)